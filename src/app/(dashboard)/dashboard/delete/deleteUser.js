...
export default function DeletUser() {
            ...
            return (
                <div className='w-full min-w-[600px] h-auto bg-gray-700 rounded-md text-center'>
                    <ul className='grid grid-cols-5 text-gray-50 font-bold p-2 border-b-2 border-gray-500 border-solid'>
                        <li>id</li>
                        <li>الاسم</li>
                        <li className='col-start-3 col-end-5'>البريد</li>
                        <li></li>
                        ...
                        data?.map((el) => {
                            return (
                                <ul key={el.id} className='w-full min-w-96 h-auto grid grid-cols-5 text-gray-200 p-2 hover:bg-gray-600 items-center relative'>
                                    <li>{el.id}</li>
                                    <li>{el.name}</li>
                                    <li className='col-start-3 col-end-5'>{el.email}</li>
    ...
    useEffect(() => {
        ...
        axios({
            url: `${Domin}/api/admin/showDeleted/user`,
            method: 'get',
            headers: {
    ...
    function restUser(id){ 
        setLoader(true)

        axios({
            method:'get',
            url:`${Domin}/api/admin/restore/user/${id}`,
            headers:{
                'Authorization':`Bearer ${userData.token}`
            }
        }).then(()=>{
            setLoader(false)
            location.reload()
            return toast.success("تم استرجاع البيانات")
        }).catch(()=>{
            setLoader(false)
    ...
    function deleteUser(id){
            ...
            axios({
                method:'delete',
                url:`${Domin}/api/admin/forceDelete/user/${id}`,
                headers:{
                    'Authorization':`Bearer ${userData.token}`
                }
            }).then(()=>{
                setLoader(false)
                location.reload()
                return toast.success("  تم حذف البيانات نهائيا")
            }).catch(()=>{
                setLoader(false)
                return toast.error("حدث خطا ما")
            })
    ...
    return (
        <div className="w-full overflow-auto">
            <p className="mb-2 font-bold text-2xl text-gray-200"># المستخدمين</p>
            {getData()}
            {funLoader()}
        </div>
    )
