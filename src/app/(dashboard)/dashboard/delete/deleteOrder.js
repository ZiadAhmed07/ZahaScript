...
export default function DeletOrder(){
    ...
    useEffect(()=>{
        axios({
            method:'get',
            url:`${Domin}/api/admin/showDeleted/order`,
            headers:{
                            ...
                            return (
                                <ul key={el.id} className='w-full min-w-96 h-auto grid grid-cols-5 text-gray-200 p-2 hover:bg-gray-600 items-center relative'>
                                    ...
                                    <li className="flex items-center justify-end gap-3 pl-2">
                                        <button onClick={()=>{deleteOrder(el.id)}}>
                                        ...
                                        <button onClick={()=>{restOrder(el.id)}}>
    ...
    function deleteOrder(id){
        ...
        axios({
            method:'delete',
            url:`${Domin}/api/admin/forceDelete/order/${id}`,
            headers:{
                'Authorization':`Bearer ${userData.token}`
            }
        }).then((res)=>{
    ...
    function restOrder(id){
        ...
        axios({
            method:'get',
            url:`${Domin}/api/admin/restore/order/${id}`,
            headers:{
                'Authorization':`Bearer ${userData.token}`
            }
        }).then(()=>{
    ...
    return(
        <div className="w-full overflow-auto">
            <p className="mb-2 font-bold text-2xl text-gray-200"># طلبات المشاريع</p>
            {getData()}
            {funLoader()}
        </div>
    )
