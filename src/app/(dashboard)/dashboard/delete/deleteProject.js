...
export default function DeleteProject(){
    ...
    useEffect(()=>{
        axios({
            method:'get',
            url:`${Domin}/api/admin/showDeleted/project`,
            headers:{
            ...
            return (
                <div className='w-full min-w-[600px] h-auto bg-gray-700 rounded-md text-center'>
                    <ul className='grid grid-cols-5 text-gray-50 font-bold p-2 border-b-2 border-gray-500 border-solid'>
                        <li>id</li>
                        <li>المشروع</li>
                        <li className="col-start-3 col-end-5">الوصف</li>
                    </ul>
                    {
                        data?.map((el) => {
                            return (
                                <ul key={el.id} className='w-full min-w-96 h-auto grid grid-cols-5 text-gray-200 p-2 hover:bg-gray-600 items-center relative'>
                                    <li>{el.id}</li>
                                    <li>{el.nameProject}</li>
                                    <li className="col-start-3 col-end-5 max-h-[40px] overflow-hidden px-2"><p className="truncate">{el.description}</p></li>
                                    <li className="flex items-center justify-end gap-3 pl-2">
                                        <button onClick={()=>{deleteProject(el.id)}}>
                                        ...
                                        <button onClick={()=>{restProject(el.id)}}>
    ...
    function deleteProject(id){
        ...
        axios({
            ...
            location.reload()
            return toast.success("تم حذف المشروع نهائيا")
        }).catch(()=>{
    ...
    function restProject(id){
        ...
        axios({
            ...
            location.reload()
            return toast.success("تم اعاده المشروع بنجاح")
        }).catch(()=>{
            return toast.error("حدث خطا ما !")
        })
    }
    ...
    return(
        <div className="w-full overflow-auto">
            <p className="mb-2 font-bold text-2xl text-gray-200"># المشاريع</p>
            {getData()}
            {funLoader()}
        </div>
    )
