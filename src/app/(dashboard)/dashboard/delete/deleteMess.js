...
export default function DeleteMess(){
    ...
    useEffect(()=>{
        axios({
            method:'get',
            url:`${Domin}/api/admin/showDeleted/contact`,
            headers:{
            ...
            return (
                <div className='w-full min-w-[600px] h-auto bg-gray-700 rounded-md text-center'>
                    <ul className='grid grid-cols-5 text-gray-50 font-bold p-2 border-b-2 border-gray-500 border-solid'>
                        <li>id</li>
                        <li>الاسم</li>
                        <li className="col-start-3 col-end-5">الرساله</li>
                        ...
                        data?.map((el) => {
                            return (
                                <ul key={el.id} className='w-full min-w-96 h-auto grid grid-cols-5 text-gray-200 p-2 hover:bg-gray-600 items-center relative'>
                                    <li>{el.id}</li>
                                    <li>{el.user?.name}</li>
                                    <li className="col-start-3 col-end-5 max-h-[40px] overflow-hidden px-2"><p className="truncate">{el.message}</p></li>
                                    <li className="flex items-center justify-end gap-3 pl-2">
                                        <button onClick={()=>{deleteMess(el.id)}}>
    ...
    function deleteMess(id){
        ...
        axios({
            method:'delete',
            url:`${Domin}/api/admin/forceDelete/contact/${id}`,
            headers:{
                'Authorization':`Bearer ${userData.token}`
            }
        }).then((res)=>{
            location.reload()
            return toast.success("تم حذف الرساله نهائيا")
        }).catch(()=>{
    ...
    function restMess(id){
        setLoader(true)

        axios({
            method:'get',
            url:`${Domin}/api/admin/restore/contact/${id}`,
            headers:{
                'Authorization':`Bearer ${userData.token}`
            }
        }).then(()=>{
            location.reload()
            return toast.success("تم اعاده الرساله بنجاح")
        }).catch(()=>{
            return toast.error("حدث خطا ما !")
        })
    }
    ...
    return(
        <div className="w-full overflow-auto">
            <p className="mb-2 font-bold text-2xl text-gray-200"># الرسايل</p>
            {getData()}
            {funLoader()}
        </div>
    )
